...
function forum_item_link_replace_callback($match){
	...
	if($match[1]){
		...
		if($item_info!=-1){
			$ret.="<a class='item_link' href='?item=".$match[1]."' onmousemove='g_moveTooltip()' style='color:".$GLOBALS['g_color'][$item_info[1]]."' onmouseover='g_showItemWithoutPreview(".$match[1].")' onmouseout = 'g_hideTooltip();'>".$item_info[0]."</a>";
		}
